...
gulp.task('bootstrap-images', function() {
  return gulp.src([
      paths.app + "/libs/bootstrap/images/**/*"
    ])
    .pipe(gulp.dest(paths.build + '/libs/bootstrap/images'));
});
gulp.task('bootstrap-css', ['bootstrap-images'],function() {
  return gulp.src([
      paths.app + "/libs/bootstrap/**/*.css"
    ])
    .pipe(mincss())
    .pipe(rename('style-dark.min.css'))
    .pipe(gulp.dest(paths.build + '/libs/bootstrap'))
    .pipe(reload({stream:true}));
});

gulp.task('jquery-filer-css',['jquery-filter-fonts','modal-css','bootstrap-css'], function(){
  return gulp.src([
